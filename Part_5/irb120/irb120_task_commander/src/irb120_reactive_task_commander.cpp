...
int main(int argc, char** argv) {
    ...
    // FIND OBJECT FIRST so that while loop condition can be satisfied
    ...
    // KEEP MOVING GEAR UNTIL IT IS WITHIN REASONABLE MARGIN OF ERROR OF GOAL LOCATION
    while (abs(g_perceived_object_pose.pose.position.x - goal_x) > 0.03 || abs(g_perceived_object_pose.pose.position.y - goal_y > 0.03)) {


	    // ******************* STEP 1: HOVER ABOVE PART (doesn't matter exactly where) *******************
	
	    // set the origin position with x and y coordinates of gear part and z high to hover above part
	    ...
	    if (!pCartTrajPlanner->plan_cartesian_path_w_rot_interp(g_q_vec_arm_Xd, goal_flange_affine, nsteps, optimal_path)) {
	    ...
	    // ******************* STEP 2: ROBOT ARM GOES TO X OFFSET POSITION NEXT TO GEAR PART *******************
	
	    // designate offset based on position of gear part relative to goal 
	    if (g_perceived_object_pose.pose.position.x < goal_x){
		flange_origin <<g_perceived_object_pose.pose.position.x - offset, g_perceived_object_pose.pose.position.y, 0.0;
	    }
	    else {
		flange_origin <<g_perceived_object_pose.pose.position.x + offset, g_perceived_object_pose.pose.position.y, 0.0;
            }
	    ...
	    // ******************* STEP 3: PUSH GEAR PART TO THE X POSITION OF GOAL *******************

	    if (g_perceived_object_pose.pose.position.x < goal_x){
	    	flange_origin <<goal_x - offset, g_perceived_object_pose.pose.position.y, 0.0;
	    }
	    else {
                flange_origin <<goal_x + offset, g_perceived_object_pose.pose.position.y, 0.0;
	    }
	    ...
	    // ******************* STEP 4: GO BACK UP AND HOVER ABOVE GEAR PART (doesn't matter exactly where) *******************
	    ...
	    flange_origin <<goal_x, g_perceived_object_pose.pose.position.y, 0.5;
	    goal_flange_affine.translation() = flange_origin; //make this part of the flange  affine description
	    ROS_INFO_STREAM("move to flange origin= " << goal_flange_affine.translation().transpose() << endl);
	    ROS_INFO_STREAM("with orientation: " << endl << goal_flange_affine.linear() << endl);
	    ...
	    // ******************* STEP 5: ROBOT ARM GOES TO Y OFFSET POSITION NEXT TO GEAR PART *******************

	    // designate the offset for the y position of the gear part	
	    if (g_perceived_object_pose.pose.position.y < goal_y) {
	    	flange_origin <<goal_x, g_perceived_object_pose.pose.position.y - offset, 0.0;
	    }
	    else {
            	flange_origin <<goal_x, g_perceived_object_pose.pose.position.y + offset, 0.0;
	    }
	    ...
	    // ******************* STEP 6: PUSH GEAR PART TO THE Y POSITION OF GOAL *******************

            if (g_perceived_object_pose.pose.position.y < goal_y) {
	    	flange_origin <<goal_x, goal_y - offset, 0.0;
	    }
	    else {
            	flange_origin <<goal_x, goal_y + offset, 0.0;
	    }
	    ...
	    // ******************* STEP 7: HOVER ABOVE GEAR PART *******************

	    flange_origin << goal_x, goal_y, 0.5;
	    ...
	    // ******************* STEP 8: FIND OBJECT LOCATION AGAIN SO THAT WHILE LOOP CONDITION CAN BE CHECKED *******************
