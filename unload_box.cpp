...
int main(int argc, char** argv) {
	...
	// LOOP THROUGH ORPHANS TO FIND COORDINATES THAT MATCH WITH BAD PART'S COORDINATES
	for (int i = 0; i < orphan_models_wrt_world.size(); i++)
	{
	    if (abs(orphan_models_wrt_world[i].pose.position.x - current_part.pose.pose.position.x) < .01
  	 	&& abs(orphan_models_wrt_world[i].pose.position.y - current_part.pose.pose.position.y) < .01
		&& abs(orphan_models_wrt_world[i].pose.position.z - current_part.pose.pose.position.z) < .01)
	    {
    ...
    for (int i=0;i<nparts;i++) {
       ROS_INFO_STREAM("orphaned  parts: "<<orphan_models_wrt_world[i]<<endl);
       // REMOVE ALL THE OTHER PARTS IN THE BOX
       //void model_to_part(osrf_gear::Model model, inventory_msgs::Part &part, unsigned short int location) 
